...
.PHONY: ci
ci:
	@$(BASH_TEST) "PATH=/usr/local/bundle/bin:/srv/root/.nvm/versions/node/v10.17.0/bin:/usr/local/bundle/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin bin/rails webpacker:install webpacker:install:react generate react:install;bin/rails assets:precompile;bin/rails db:setup db:migrate ci"
