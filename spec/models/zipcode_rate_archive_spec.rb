...
RSpec.describe ZipcodeRateArchive, type: :model do
  let(:user) { User.first }
  ...
  describe 'archive zipcode rates' do
    before(:each) do
      # version 1
      create_production_version
      create :zipcode_rate, version: current_production_number
    ...
    context 'when successful' do
      ...
      it 'does not archive preview versions greater than current production', js: true do
        ...
        create :zipcode_rate, version: current_preview_number
    ...
    context 'when not successful' do
      it 'returns an error message' do
        error_message = 'BOOM!'
        allow(ZipcodeRateArchive).to receive(:create_archives).and_raise(StandardError, error_message)
      ...
      it 'logs errors at the database level' do
        ...
        allow(ZipcodeRateArchive).to receive(:create_archives).and_raise(statement_invalid)
  ...
  def archive_test(initial_count:,
                   ...
                   archive_count:)
    expect(ZipcodeRate.count).to eq(initial_count)
    expect(ZipcodeRateArchive.count).to eq(0)

    ZipcodeRateArchive.archive_previous_versions

    expect(ZipcodeRate.count).to eq(count_total)
    expect(ZipcodeRate.where('version >= ?', current_production_number).size)
      .to eq(count_greater_equal_production)

    expect(ZipcodeRateArchive.count).to eq(archive_count)
    expect(ZipcodeRateArchive.where('version < ?', current_production_number).size).to eq(archive_count)
  end
