  ...
  describe 'Number' do
    context 'is_i?' do
      it 'is true when a number is a fixnum' do
        expect(DS::Number.is_i?(16_013)).to be_truthy
      end
      ...
      it 'is false when a number is not a fixnum' do
        expect(DS::Number.is_i?(16_013.00)).to be_falsy
    ...
    context 'is_f?' do
      it 'is true when a number is a float' do
        expect(DS::Number.is_f?(16_013.00)).to be_truthy
      end

      it 'is true when a number is an integer' do
        expect(DS::Number.is_f?(16_013)).to be_truthy
      end
