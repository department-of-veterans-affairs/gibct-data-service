...
RSpec.describe Version, type: :model do
  describe 'buildable?' do
    ...
    let(:upload_dates_after) { [after_preview_date] * CSV_TYPES_REQUIRED_TABLE_NAMES.length }
    let(:upload_dates_before) { [before_preview_date] * CSV_TYPES_REQUIRED_TABLE_NAMES.length }

    it 'returns false if upload dates is less than institution table count' do
      allow(Upload).to receive_message_chain(:last_uploads, :to_a, :select, :map)
