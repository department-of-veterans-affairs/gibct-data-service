...
RSpec.describe Group, type: :model do
  ...
  describe 'sheet_names' do
    it 'has classes names from config/group_types' do
      sheet_names = Group.group_config_options(group.csv_type)[:types]&.map(&:name)
  ...
  describe 'sheets' do
    it 'has classes from config/group_types' do
      sheets = Group.group_config_options(group.csv_type)[:types]
  ...
  describe 'group_config_options' do
    it 'returns group type config' do
      ...
      expect(Group.group_config_options(group_type)[:klass]).to eq(group_type)
    end
  end

  describe 'export_as_zip' do
    it 'returns binary_data' do
      binary_data = Group.export_as_zip('Accreditation')
