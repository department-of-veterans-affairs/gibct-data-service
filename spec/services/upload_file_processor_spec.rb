...
RSpec.describe UploadFileProcessor do
  ...
  describe '#load_file' do
    ...
    context 'when async disabled' do
      ...
      it 'calls #file_from_blob and returns false' do
        expect(service).to receive(:file_from_blob).and_return(false)
        service.load_file
      end
      ...
      it 'calls klass#load_with_roo with non-async file options' do
        ...
        expect(klass_double).to receive(:load_with_roo).with(upload.upload_file.tempfile, file_options)
        service.load_file
      end
    ...
    context 'when async enabled' do
      ...
      it 'calls #file_from_blob and returns tempfile' do
        allow(Tempfile).to receive(:new).and_return(upload.upload_file.tempfile)
        expect(service).to receive(:file_from_blob).and_return(upload.upload_file.tempfile)
        service.load_file
      end
      ...
      it 'calls klass#load_with_roo with async file options' do
        ...
        expect(klass_double).to receive(:load_with_roo).with(upload.upload_file.tempfile, async_file_options)
        service.load_file
      end
