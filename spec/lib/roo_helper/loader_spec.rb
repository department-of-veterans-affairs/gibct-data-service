...
describe RooHelper::Loader do
  ...
  describe 'merge_options' do
    it 'sets defaults' do
      ...
      sheet1 = merged_options[:sheets][0]
      expect(sheet1[:skip_lines]).to eq(0)
      expect(sheet1[:first_line]).to eq(2)
      expect(sheet1[:klass]).to eq(edu_program)
    end
    ...
    it 'does not override sheets objects' do
      ...
      sheet1 = merged_options[:sheets][0]
      expect(sheet1[:skip_lines]).to eq(2)
      expect(sheet1[:first_line]).to eq(3)
      expect(sheet1[:klass]).to eq(program)
    end
