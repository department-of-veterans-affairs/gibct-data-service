...
RUN apk update
RUN apk add -y git make gcc-c++ openssl-devel readline-devel zlib-devel sqlite-devel postgresql-devel socat timeout epel-release nc

# Install Red Hat SCI library for ruby packages
RUN apk add -y centos-release-scl-rh
RUN apk add -y rh-ruby23 rh-ruby23-ruby-devel rh-ruby23-rubygems
RUN echo "source /opt/rh/rh-ruby23/enable" > /etc/profile.d/rh-ruby23.sh
RUN ["/bin/bash", "--login", "-c", "gem install --no-doc bundler"]

# install phantomjs
RUN apk add -y fontconfig freetype freetype-devel fontconfig-devel libstdc++ && \
