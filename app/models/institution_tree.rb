...
module InstitutionTree
  ...
  def self.build_branches(institutions)
    branches = []
  ...
  def self.build_extensions(institutions, branch_facility_code)
    extensions = []
    institutions.each do |institution|
  ...
  def self.main_campus(facility_code, version)
  ...
  def self.descendants(facility_code, version)
    str = <<-SQL
