...
class ZipcodeRateArchive < ZipcodeRate
  ...
  def self.archive_previous_versions
    ...
    begin
      ActiveRecord::Base.transaction do
        ...
        ZipcodeRate.where('version >= ? and version < ?', previous_version, production_version).delete_all
      end
  ...
  def self.create_archives(previous_version, production_version)
    str = <<-SQL
      INSERT INTO zipcode_rates_archives
        SELECT *
          FROM zipcode_rates
