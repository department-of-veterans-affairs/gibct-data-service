...
class DataCsv < ActiveRecord::Base
  ...
  def self.complete?
    CsvFile::STI.keys.inject(true) do |s, a|
      ...
      s && store.present? && store.data_store.present?
    end
  end
  ...
  def self.gibct_institution_column_names
    GibctInstitution.column_names - %w(id created_at updated_at)
  end
  ...
  def self.map_value_to_type(value, type)
    ...
    when :boolean
      %w(true t yes y 1 on).include?(value.to_s.downcase) unless value.nil?
    else
      value
    end
  end
  ...
  def self.update_with_accreditation
    ...
    # TODO: Turn this into a template or ORM call
    query_str = 'UPDATE data_csvs SET '
