...
class Complaint < ActiveRecord::Base
  ...
  def lowercase_inclusion_validator(attribute, collection, blank_ok = true)
    ...
    unless collection.include?(var.try(:downcase))
  ...
  def self.update_sums_by_fac
    ...
    Complaint.transaction do
      results.each do |result|
        attributes = fac_code_sums.inject({}) do |a, t|
          a[t] = result[t]
          a
        end
  ...
  def self.update_ope_from_crosswalk
    Complaint.all.find_each do |c|
      crosswalk = VaCrosswalk.find_by(facility_code: c.facility_code)
  ...
  def self.update_sums_by_ope6
    ...
    DataCsv.transaction do
      results.each do |result|
        attributes = ope6_sum_terms.inject({}) do |a, t|
          a[t] = result[t]
          a
        end
