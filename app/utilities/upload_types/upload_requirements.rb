class UploadRequirements
  ...
  def self.requirements_messages(type)
  ...
  def self.validation_messages_inclusion(type)
    inclusion = []
  ...
  def self.klass_validator(validation_class, type)
    type.validators.map do |validations|
      affected_attributes(validations, type) if validation_class == validations.class
    end.flatten.compact
  end

  def self.validation_messages_presence(type)
  ...
  def self.validation_messages_numericality(type)
  ...
  def self.validation_messages_uniqueness(type)
  ...
  def self.affected_attributes(validations, type)
